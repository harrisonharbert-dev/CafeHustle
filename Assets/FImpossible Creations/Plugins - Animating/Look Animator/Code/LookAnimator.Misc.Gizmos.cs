    ...
    public partial class FLookAnimator
    {
        ...
        private void Gizmos_DrawFeatureGuides(float radius)
        {
            ...
            if (StopLookingAbove < 180)
            {
                ...
                float stopLookAbove = StopLookingAbove;
                if (LookState == EFHeadLookState.OutOfMaxRotation) stopLookAbove -= StopLookAboveMargin;

                Handles.DrawWireArc(Vector3.zero, ModelUpAxis, ModelForwardAxis, stopLookAbove, radius / 1.5f);
                Handles.DrawWireArc(Vector3.zero, ModelUpAxis, ModelForwardAxis, -stopLookAbove, radius / 1.5f);
                ...
                Handles.DrawSolidArc(Vector3.zero, ModelUpAxis, ModelForwardAxis, stopLookAbove, radius / 1.5f);
                Handles.DrawSolidArc(Vector3.zero, ModelUpAxis, ModelForwardAxis, -stopLookAbove, radius / 1.5f);
            }
